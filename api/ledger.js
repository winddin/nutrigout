  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (type === 'create_trip') {
        ...
        // Only send columns that exist in shopping_trips table
        const tripRow = {
          id:        trip.id,
          user_id:   userId,
        ...
        const saved = await sbPost('shopping_trips', tripRow);
        const savedTrip = Array.isArray(saved) ? saved[0] : saved;
        return res.status(200).json({ ok: true, trip: { ...savedTrip, items: [] } });
      }
      ...
      if (type === 'update_sort_order') {
        ...
        // Patch sort_order only — parallel but lightweight (single field update each)
        await Promise.all(
          items.map(({ id, sort_order }) =>
            sbPatch('inventory', `?id=eq.${encodeURIComponent(id)}&user_id=eq.${userId}`, { sort_order })
          )
        );
